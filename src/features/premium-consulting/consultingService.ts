...
export function sendUserConsultingMessage(
  uid: string,
  userName: string,
  userEmail: string,
  content: string
  ...
  const session = createOrUpdateUserSession(uid, userName, userEmail, userMsg)

  const hasSystemAck = session.messages.some((m) => m.role === 'system')
  if (!hasSystemAck) {
    createOrUpdateUserSession(uid, userName, userEmail, {
      role: 'system',
      content:
